...
class App extends Component {
  ...
  deleteCharHandler = (index) => {
    ...
    const updatedInputText = charArray.join('');
    this.setState({inputText: updatedInputText});
  }
  render() {
    const charList = this.state.inputText.split('').map((character,index) => {
      return(
        <CharComponent value={character} key={index} clicked={()=>this.deleteCharHandler(index)}/>
      )
    });
    return (
      <div className="App">
        ...
        <p>Length of the above input: {this.state.inputText.length}</p>
